...
{
public:
	BiSortTree( );  //{root = Creat(root);}         //构造函数，建立一棵二叉排序树
	BiSortTree(T r[ ], int n);                     //创建一棵二叉排序树 
	~BiSortTree( );  //{Release(root);}          //析构函数，释放各结点的存储空间
	void PreOrder( );  //{PreOrder(root);}     //前序遍历二叉树
	void InOrder( ){InOrder(root);}       //中序遍历二叉树
	void PostOrder( ){PostOrder(root);}   //后序遍历二叉树
	void LeverOrder( );                 //层序遍历二叉树
	BiNode<T> *GetRoot();                //获取根 
	BiNode<T> *SearchBST(BiNode<T> *&root, T k);   //在二叉排序树中查找k 
	bool DeleteBST(BiNode<T> *&root, T k);   //在二叉排序树中删除k 
	
	
private:
	BiNode<T> *root;                             //指向根结点的头指针
	BiNode<T> *Creat(BiNode<T> *bt);       //构造函数调用
	void InsertBST(BiNode<T> *&root, BiNode<T> *&s);   //在二叉排序树中插入结点s 
